...
const isSpectating = computed(() => {
  return (
    ...
    !!viewedPlayer.value &&
    viewedPlayer.value.playerId !== myPlayer.value.playerId
  )
})
...
const canManageTeam = computed(
  () => props.state?.phase === 'PLANNING' && !isDead.value && !isSpectating.value,
)
...
const ownedLineIds = computed((): Set<string> => {
  ...
  units.forEach((unit) => {
    const identity =
      lineIdentity(unit?.lineId) ?? lineIdentity(unit?.definitionId) ?? lineIdentity(unit?.name)
    if (identity) {
...
const onBenchDragStart = (evt: DragEvent, unit: GameUnit) => {
  ...
  if (evt.dataTransfer) {
    ...
    const img = (evt.currentTarget as HTMLElement)?.querySelector<HTMLImageElement>(
      '.bench-unit-img',
    )
    benchDragPreviewCleanup = setUnitDragPreview(
      evt,
      img ?? getUnitIconPath(unit.definitionId, props.state?.gameMode),
    )
  }
}
...
const handleBoardMove = (movePayload: MovePayload) => {
  ...
  emit('action', {
    type: 'MOVE',
    unitId: movePayload.unitId,
    targetX: movePayload.x,
    targetY: movePayload.y,
    playerId: myPlayer.value?.playerId,
  })
}
...
function findOwnedUnit(unitId: string): GameUnit | null {
  return (
    [...benchUnits.value, ...myPlayerBoardUnits.value].find((unit) => unit?.id === unitId) ?? null
  )
}
...
const timerFillPercent = computed(() => {
  ...
  const duration = Math.max(
    1,
    props.state.totalPhaseDuration || (props.state.phase === 'PLANNING' ? 8000 : 20000),
  )
...
const timerFillColor = computed(() => {
  ...
  if (props.state.phase === 'PLANNING' && props.state.planningPauseReason === 'AUGMENT_SELECTION')
    return '#67e8f9'
  if (props.state.phase === 'PLANNING' && props.state.planningPauseReason === 'SOLO_READY')
    return '#22c55e'
...
<template>
  <div class="game-interface">
    ...
    <template v-if="state">
            ...
            <div class="room-details">
              ...
              >
                <svg viewBox="0 0 24 24" aria-hidden="true">
                  <path
                    d="M4 4.75A1.75 1.75 0 0 1 5.75 3h9.5A1.75 1.75 0 0 1 17 4.75v14.5A1.75 1.75 0 0 1 15.25 21h-9.5A1.75 1.75 0 0 1 4 19.25v-14.5Z"
                  />
      ...
      <div class="bottom-ui" v-if="myPlayer && !isSpectating" :class="{ 'dead-state': isDead }">
        ...
        <div class="stats-panel">
          <div class="level-info">
                  ...
                  <div class="odds-grid">
                    <div v-for="(prob, tier) in currentShopOdds" :key="tier" class="odds-row">
                      <span
                        ...
                        :style="{ backgroundColor: rarityColors[tier] }"
                      ></span>
            ...
            <div
              ...
              :title="
                myPlayer.level >= 9
                  ? 'Maximum level'
                  : `XP: ${myPlayer.xp} / ${myPlayer.nextLevelXp}`
              "
            >
              <div
                class="xp-fill"
                :style="{
                  width:
                    myPlayer.level >= 9 ? '100%' : (myPlayer.xp / myPlayer.nextLevelXp) * 100 + '%',
                }"
              ></div>
              <span class="xp-text">{{
                myPlayer.level >= 9 ? 'MAX' : `${myPlayer.xp} / ${myPlayer.nextLevelXp} XP`
              }}</span>
            </div>
          </div>
          <div class="stats-row">
            ...
            <div
              class="unit-count"
              :class="{ 'max-units': myPlayerBoardUnits.length >= myPlayer.level }"
          ...
          <button
            class="xp-btn"
            @click="buyXp"
            :disabled="!canManageShopAndBench || myPlayer.gold < 4 || myPlayer.level >= 9"
          >
        ...
        <div class="bench-area-wrapper">
            ...
            <div class="bench-slots">
              ...
              >
                ...
                >
                  ...
                  <div
                    class="bench-unit-inner"
                    :style="{ borderColor: getRarityColor(slot.unit.cost) }"
                  >
                    <div v-if="isStarringUp(slot.unit.id)" class="star-up-burst">
                      <span
                        ...
                        :key="j"
                        class="star-particle"
                        :style="{ '--particle-index': j }"
                      ></span>
                    </div>
                    <img
                      :src="getUnitIconPath(slot.unit.definitionId, props.state?.gameMode)"
          ...
          <div
            class="sell-zone bench-sell-zone"
          ...
          >
            <div class="sell-content">
              ...
              <span class="sell-text">{{
                draggedUnit ? 'SELL UNIT FOR' : 'DRAG HERE TO SELL'
              }}</span>
        ...
        <div class="shop-area">
          ...
          <div class="shop-cards">
            <div
              ...
              :class="{
                empty: !card,
                'can-buy': canManageShopAndBench && card && myPlayer.gold >= card.cost,
                'owned-line': card && isShopCardOwned(card),
                [`rarity-${card?.cost || 1}`]: card,
              }"
                ...
                (e) =>
                  card
                    ? handleShowTooltip(
                        ...
                        card,
                        'top',
                        idx === 4 ? 'more-left' : idx === 3 ? 'left' : undefined,
                      )
                    : null
              "
              @mouseleave="handleHideTooltip"
            >
              <template v-if="card">
                <div class="shop-card-portrait">
                  <img
                    :src="getUnitIconPath(card.id, props.state?.gameMode)"
                    class="shop-card-img"
                    draggable="false"
                  />
                </div>
      ...
      <div v-else-if="!myPlayer" class="waiting-message">Waiting for player data...</div>
    </template>

    <div v-else class="waiting-message">Waiting for game state...</div>

    <div
      v-if="isAbandonDialogVisible"
      class="abandon-dialog-backdrop"
      @click.self="cancelAbandonGame"
    >
      <section
      ...
      >
        ...
        <p>
          You will be removed from this match and returned to the home screen. This cannot be
          undone.
        </p>
        <div class="abandon-dialog__actions">
          <button class="abandon-dialog__cancel" type="button" @click="cancelAbandonGame">
            Cancel
          </button>
          <button class="abandon-dialog__confirm" type="button" @click="confirmAbandonGame">
            Abandon Game
          </button>
        </div>
      </section>
...
.sell-zone.hovered {
  ...
  background: linear-gradient(
    90deg,
    rgba(127, 29, 29, 0.24) 0%,
    rgba(239, 68, 68, 0.34) 50%,
    rgba(127, 29, 29, 0.24) 100%
  );
  box-shadow:
    0 0 24px rgba(248, 113, 113, 0.42),
...
.shop-card.rarity-1.can-buy:hover {
  border-color: #94a3b8;
  box-shadow: 0 0 10px rgba(148, 163, 184, 0.3);
}
.shop-card.rarity-2.can-buy:hover {
  border-color: #4ade80;
  box-shadow: 0 0 10px rgba(74, 222, 128, 0.3);
}
.shop-card.rarity-3.can-buy:hover {
  border-color: #60a5fa;
  box-shadow: 0 0 10px rgba(96, 165, 250, 0.3);
}
.shop-card.rarity-4.can-buy:hover {
  border-color: #c084fc;
  box-shadow: 0 0 10px rgba(192, 132, 252, 0.3);
}
.shop-card.rarity-5.can-buy:hover {
  border-color: #fbbf24;
  box-shadow: 0 0 10px rgba(251, 191, 36, 0.3);
}

.shop-card.owned-line.rarity-1.can-buy:hover {
  box-shadow:
    0 0 10px rgba(148, 163, 184, 0.3),
    inset 0 0 0 2px rgba(34, 211, 238, 0.95),
    0 0 18px rgba(34, 211, 238, 0.35);
}
.shop-card.owned-line.rarity-2.can-buy:hover {
  box-shadow:
    0 0 10px rgba(74, 222, 128, 0.3),
    inset 0 0 0 2px rgba(34, 211, 238, 0.95),
    0 0 18px rgba(34, 211, 238, 0.35);
}
.shop-card.owned-line.rarity-3.can-buy:hover {
  box-shadow:
    0 0 10px rgba(96, 165, 250, 0.3),
    inset 0 0 0 2px rgba(34, 211, 238, 0.95),
    0 0 18px rgba(34, 211, 238, 0.35);
}
.shop-card.owned-line.rarity-4.can-buy:hover {
  box-shadow:
    0 0 10px rgba(192, 132, 252, 0.3),
    inset 0 0 0 2px rgba(34, 211, 238, 0.95),
    0 0 18px rgba(34, 211, 238, 0.35);
}
.shop-card.owned-line.rarity-5.can-buy:hover {
  box-shadow:
